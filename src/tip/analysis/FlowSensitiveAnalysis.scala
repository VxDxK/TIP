...
object FlowSensitiveAnalysis {
    ...
    options match {
      ...
      case AnalysisOption.`simple` =>
        Some(kind match {
          ...
          case Analysis.variablesize => new VariableSizeAnalysis.Intraprocedural.SimpleSolver(typedCfg.left.get)
      ...
      case AnalysisOption.`wl` =>
        Some(kind match {
          ...
          case Analysis.variablesize => new VariableSizeAnalysis.Intraprocedural.WorklistSolver(typedCfg.left.get)
          case _ => throw new RuntimeException(s"Unsupported solver option `$options` for the analysis $kind")
        })
  ...
  object Analysis extends Enumeration {
    val sign, livevars, available, vbusy, reaching, constprop, interval, variablesize, copyconstprop, uninitvars, taint = Value
  }
}
